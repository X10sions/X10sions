...
namespace Dapper.Tests.Performance {
  ...
  public class LegacyTests {
    ...
    private class Tests : List<Test> {
      public void Add(Action<int> iteration, string name) => Add(new Test(iteration, name));

      public void AsyncAdd(Func<int, Task> iterationAsync, string name) => Add(new Test(iterationAsync, name));

      public async Task RunAsync(int iterations) {
        // warmup
        foreach(var test in this) {
          test.Iteration?.Invoke(iterations + 1);
          if(test.IterationAsync != null)
            await test.IterationAsync(iterations + 1).ConfigureAwait(false);
        ...
        for(var i = 1; i <= iterations; i++) {
          foreach(var test in this.OrderBy(ignore => rand.Next())) {
            ...
            if(test.IterationAsync != null)
              await test.IterationAsync(i).ConfigureAwait(false);
            test.Watch.Stop();
          }
        }
    ...
    private static DataClassesDataContext GetL2SContext(SqlConnection connection) => new DataClassesDataContext(connection);
