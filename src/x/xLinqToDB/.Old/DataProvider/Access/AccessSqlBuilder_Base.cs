...
//  public abstract class AccessSqlBuilder_Base : BasicSqlBuilder {
//    public AccessSqlBuilder_Base(ISqlOptimizer sqlOptimizer, SqlProviderFlags sqlProviderFlags, ValueToSqlConverter valueToSqlConverter)
...
//        StringBuilder.AppendLine($" ALTER COLUMN {Convert(field.PhysicalName, ConvertType.NameToQueryField)} COUNTER(1,1)");
//      } else {
...
//          StringBuilder.Append($" TOP {int.MaxValue}");
...
//    protected override bool ParenthesizeJoin(List<SqlJoinedTable> tsJoins) => true;
...
//        p.ReplaceLike = !predicate.IsSqlLike;
//      }
...
//        case '|':
//        case '^':
//          throw new SqlException("Operator '{0}' is not supported by the {1}.", expr.Operation, GetType().Name);
//      }
...
//          break;
//        case "convert":
//          switch(ReflectionExtensions.ToUnderlying(func.SystemType).GetTypeCodeEx()) {
//            case TypeCode.String:
//              func = new SqlFunction(func.SystemType, "CStr", func.Parameters[1]);
//              break;
...
//      if(len < 3)
//        throw new SqlException($"CASE statement is not supported by the {GetType().Name}.");
//      if(len == 3)
...
//    protected override void BuildCreateTableIdentityAttribute2(SqlField field) => StringBuilder.Append("IDENTITY");
