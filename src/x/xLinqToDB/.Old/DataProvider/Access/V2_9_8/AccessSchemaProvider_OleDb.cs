...
namespace LinqToDB.DataProvider.Access.V_2_9_8 {
  public class AccessSchemaProvider_OleDb : SchemaProviderBase {
    ...
    protected override List<ColumnInfo> GetColumns(DataConnection dataConnection) {
      ...
      return (
        ...
        select new ColumnInfo {
          ...
          IsIdentity = Converter.ChangeTypeTo<int>(c["COLUMN_FLAGS"]) == 90 && (dt == null || dt.TypeName == null || (!string.Equals(dt.TypeName, "boolean", StringComparison.OrdinalIgnoreCase) && !string.Equals(dt.TypeName, "bit", StringComparison.OrdinalIgnoreCase))),
        }
      ).ToList();
    }
    ...
    protected override List<ForeignKeyInfo> GetForeignKeys(DataConnection dataConnection) =>
      ...
      throw new NotImplementedException();//2020-03-23  var data = ((OleDbConnection)Proxy.GetUnderlyingObject((DbConnection)dataConnection.Connection)).GetOleDbSchemaTable(OleDbSchemaGuid.Foreign_Keys, null);//2020-03-23  var q = from fk in data.AsEnumerable()//2020-03-23          select new ForeignKeyInfo {//2020-03-23            Name = fk.Field<string>("FK_NAME"),//2020-03-23            ThisColumn = fk.Field<string>("FK_COLUMN_NAME"),//2020-03-23            OtherColumn = fk.Field<string>("PK_COLUMN_NAME"),//2020-03-23            ThisTableID = fk.Field<string>("FK_TABLE_CATALOG") + "." + fk.Field<string>("FK_TABLE_SCHEMA") + "." + fk.Field<string>("FK_TABLE_NAME"),//2020-03-23            OtherTableID = fk.Field<string>("PK_TABLE_CATALOG") + "." + fk.Field<string>("PK_TABLE_SCHEMA") + "." + fk.Field<string>("PK_TABLE_NAME"),//2020-03-23            Ordinal = ConvertTo<int>.From(fk.Field<long>("ORDINAL")),//2020-03-23          };//return q.ToList();
