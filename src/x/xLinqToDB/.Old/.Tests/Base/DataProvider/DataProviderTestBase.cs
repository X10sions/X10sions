...
namespace LinqToDB.Tests.Base.DataProvider {
  ...
  public class DataProviderTestBase  //:TestBase
    ...
    protected T TestType<T>(DataConnection conn, string fieldName,
      ...
      var type = typeof(T).IsGenericType && typeof(T).GetGenericTypeDefinition() == typeof(Nullable<>) ? typeof(T).GetGenericArguments()[0] : typeof(T);
      // Get NULL value.
      ...
      Assert.Equal(value, conn.MappingSchema.GetDefaultValue(typeof(T)));
