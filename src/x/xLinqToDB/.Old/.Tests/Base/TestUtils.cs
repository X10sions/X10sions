...
namespace LinqToDB.Tests.Base {
  public static partial class TestUtils {
    ...
    private static string MySqlVersion() => throw new InvalidOperationException();

    [Sql.Function("DBINFO", ServerSideOnly = true)]
    private static string DbInfo(string property) => throw new InvalidOperationException();
    ...
    private static string DbName() => throw new InvalidOperationException();
    ...
    private static string SchemaName() => throw new InvalidOperationException();
    ...
    private static string ServerName() => throw new InvalidOperationException();
    ...
    public static DateTime StripMilliseconds(DateTime value, bool fix) => fix ? new DateTime(value.Year, value.Month, value.Day, value.Hour, value.Minute, value.Second) : value;

    static TempTable<T> CreateTable<T>(IDataContext db, string? tableName) => db.CreateSqlProvider() is FirebirdSqlBuilder ? new FirebirdTempTable<T>(db, tableName) : new TempTable<T>(db, tableName);
    ...
    public static TempTable<T> CreateLocalTable<T>(this IDataContext db, IEnumerable<T> items) => CreateLocalTable(db, null, items);
