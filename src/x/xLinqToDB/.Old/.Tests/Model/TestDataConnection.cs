...
namespace LinqToDB.Tests.Model {
  ...
  public class TestDataConnection : DataConnection, ITestDataContext {
    ...
    public static IQueryable<Parent> GetParent9(ITestDataContext db, Child ch) => throw new InvalidOperationException();

    [ExpressionMethod("Expression9")]
    public IQueryable<Parent> GetParent10(Child ch) => throw new InvalidOperationException();

    static Expression<Func<ITestDataContext, Child, IQueryable<Parent>>> Expression9() => (db, ch) =>
         from p in db.Parent
