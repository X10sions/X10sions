...
public static class ITableExtensions {
  ...
  //public static int InsertIfNotExists<T>(this ITable<T> table, T obj, Expression<Func<T, bool>> predicate) where T : class => table.Any(predicate) ? 0 : table.Insert(obj);
  ...
  public static T InsertWithOutputIfNotExists<T>(this ITable<T> table, T obj, Expression<Func<T, bool>> predicate) where T : notnull => table.FirstOrDefault(predicate) ?? table.InsertWithOutput(obj);
}