...
public static class ITagHelperExtensions {
  ...
  public static async Task RunTagHelperAsync(this ITagHelper tagHelper, Options options) {
    ...
    var output = new TagHelperOutput(options.TagName, new TagHelperAttributeList(options.Attributes), async (b, e) => { var c = new DefaultTagHelperContent(); c.SetHtmlContent(options.Content); return c; }) { TagMode = options.TagMode };
