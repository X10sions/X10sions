...
namespace Microsoft.AspNetCore.Http {
  public static class HttpRequestExtensions {
    ...
    #region "DebugObject"
    public static HttpRequestDebugObject GetDebugObject(this HttpRequest httpRequest) => new HttpRequestDebugObject(httpRequest);
    #endregion

  }

  public class HttpRequestDebugObject : IDebugObject<HttpRequest> {
    public HttpRequestDebugObject(HttpRequest value, bool includeChildren = false) {
      Value = value;
    ...
    public string GetDisplayUrl => Value.GetDisplayUrl();
    public string Method => Value.Method;
    public string Scheme => Value.Scheme;
    public HostString Host => Value.Host;
    public string Host_Host => Value.Host.Host;
    public int Host_Port => Value.Host.Port.GetValueOrDefault(80);
    public string PathBase => Value.PathBase;
    public string Path => Value.Path;
    public string QueryString => Value.QueryString.ToString();
    public string Protocol => Value.Protocol;

    [Obsolete("TODO: DebugObject for Cookies")] public string Cookies => Value.Cookies.DebugString(includeChildren);

    public string VirtualDirectory() => Value.VirtualDirectory();
    public string VirtualFilePathWithExtension => Value.VirtualFilePath(true);
    public string VirtualFilePath => Value.VirtualFilePath(false);
