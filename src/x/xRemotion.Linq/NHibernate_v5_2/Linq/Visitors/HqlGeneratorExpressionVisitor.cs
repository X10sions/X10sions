...
namespace NHibernate_v5_2.Linq.Visitors {
  public class HqlGeneratorExpressionVisitor : IHqlExpressionVisitor {
    ...
    public static HqlTreeNode Visit(Expression expression, VisitorParameters parameters) => new HqlGeneratorExpressionVisitor(parameters).VisitExpression(expression);
    ...
    public HqlTreeNode Visit(Expression expression) => VisitExpression(expression);
    ...
    protected HqlTreeNode VisitExpression(Expression expression) {
      ...
      switch (expression.NodeType) {
        ...
        case ExpressionType.Extension:
          switch (expression) {
            case SubQueryExpression subQueryExpression: return VisitSubQueryExpression(subQueryExpression);
            case QuerySourceReferenceExpression querySourceReferenceExpression: return VisitQuerySourceReferenceExpression(querySourceReferenceExpression);
            case VBStringComparisonExpression vbStringComparisonExpression: return VisitVBStringComparisonExpression(vbStringComparisonExpression);
            case NhAverageExpression nhAverageExpression: return VisitNhAverage(nhAverageExpression);
            case NhMinExpression nhMinExpression: return VisitNhMin(nhMinExpression);
            case NhMaxExpression nhMaxExpression: return VisitNhMax(nhMaxExpression);
            case NhSumExpression nhSumExpression: return VisitNhSum(nhSumExpression);
            case NhCountExpression nhCountExpression: return VisitNhCount(nhCountExpression);
            case NhDistinctExpression nhDistinctExpression: return VisitNhDistinct(nhDistinctExpression);
            case NhStarExpression nhStarExpression: return VisitNhStar(nhStarExpression);
            case NhNominatedExpression nhNominatedExpression: return VisitNhNominated(nhNominatedExpression);
            //case NhNewExpression nhNewExpression: return VisitNhNew(nhNewExpression);
            default: throw new NotSupportedException(expression.ToString());
          }
    ...
    private HqlTreeNode VisitTypeBinaryExpression(TypeBinaryExpression expression) => BuildOfType(expression.Expression, expression.TypeOperand);
    ...
    internal HqlBooleanExpression BuildOfType(Expression expression, System.Type type) {
      ...
      var meta = sessionFactory.GetClassMetadata(type) as AbstractEntityPersister;
...
- map its subclasses.";
    ...
    protected HqlTreeNode VisitNhStar(NhStarExpression expression) => _hqlTreeBuilder.Star();
    ...
    protected HqlTreeNode VisitNhCount(NhCountExpression expression) => _hqlTreeBuilder.Cast(_hqlTreeBuilder.Count(VisitExpression(expression.Expression).AsExpression()), expression.Type);

    protected HqlTreeNode VisitNhMin(NhMinExpression expression) => _hqlTreeBuilder.Min(VisitExpression(expression.Expression).AsExpression());

    protected HqlTreeNode VisitNhMax(NhMaxExpression expression) => _hqlTreeBuilder.Max(VisitExpression(expression.Expression).AsExpression());

    protected HqlTreeNode VisitNhSum(NhSumExpression expression) => _hqlTreeBuilder.Cast(_hqlTreeBuilder.Sum(VisitExpression(expression.Expression).AsExpression()), expression.Type);
    ...
    protected HqlTreeNode VisitQuerySourceReferenceExpression(QuerySourceReferenceExpression expression) => _hqlTreeBuilder.Ident(_parameters.QuerySourceNamer.GetName(expression.ReferencedQuerySource));
    ...
    private HqlTreeNode VisitVBStringComparisonExpression(VBStringComparisonExpression expression) =>
      ...
      VisitExpression(expression.Comparison);
    ...
    protected HqlTreeNode VisitConstantExpression(ConstantExpression expression) {
      if (expression.Value != null) {
        var entityName = expression.Value as IEntityNameProvider;
        if (entityName != null) {
    ...
    protected HqlTreeNode VisitLambdaExpression(LambdaExpression expression) => VisitExpression(expression.Body);

    protected HqlTreeNode VisitParameterExpression(ParameterExpression expression) => _hqlTreeBuilder.Ident(expression.Name);
    ...
    protected HqlTreeNode VisitSubQueryExpression(SubQueryExpression expression) {
      var query = QueryModelVisitor.GenerateHqlQuery(expression.QueryModel, _parameters, false, null);
