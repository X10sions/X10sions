...
namespace NHibernate_v5_2.Linq.ExpressionTransformers {
  ...
  public class RemoveCharToIntConversion : IExpressionTransformer<BinaryExpression> {
    ...
    private static bool IsConvertExpression(Expression expression) => (expression.NodeType == ExpressionType.Convert);

    private static bool IsConstantExpression(Expression expression) => (expression.NodeType == ExpressionType.Constant);
