...
namespace NHibernate_v5_2.Linq.Clauses {
  public class NhWithClause : NhClauseBase, IBodyClause {
    ...
    public override string ToString() => "with " + Predicate;

    protected override void Accept(INhQueryModelVisitor visitor, QueryModel queryModel, int index) => visitor.VisitNhWithClause(this, queryModel, index);

    IBodyClause IBodyClause.Clone(CloneContext cloneContext) => Clone(cloneContext);
