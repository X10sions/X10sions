...
namespace SharpRepository.ODataRepository.Linq.QueryGeneration {
  public class ODataApiGeneratorQueryModelVisitor : QueryModelVisitorBase {
    public static CommandData GenerateODataApiQuery(QueryModel queryModel) {
      var visitor = new ODataApiGeneratorQueryModelVisitor();
      visitor.VisitQueryModel(queryModel);
      return visitor.GetODataApiCommand();
    }
    ...
    public CommandData GetODataApiCommand() => new CommandData(_queryParts);
    ...
    public override void VisitSelectClause(SelectClause selectClause, QueryModel queryModel) {
      ...
      if (string.IsNullOrEmpty(_queryParts.SelectPart)) {
        _queryParts.SelectPart = GetODataApiExpression(selectClause.Selector);
      }
      base.VisitSelectClause(selectClause, queryModel);
    }

    public override void VisitWhereClause(WhereClause whereClause, QueryModel queryModel, int index) {
      _queryParts.AddWherePart(GetODataApiExpression(whereClause.Predicate));
      base.VisitWhereClause(whereClause, queryModel, index);
    }

    public override void VisitOrderByClause(OrderByClause orderByClause, QueryModel queryModel, int index) {
      if (orderByClause.Orderings.Any()) {
        _queryParts.AddOrderByPart(GetODataApiExpression(orderByClause.Orderings[0].Expression), orderByClause.Orderings[0].OrderingDirection == OrderingDirection.Desc);
      }
    ...
    public override void VisitJoinClause(JoinClause joinClause, QueryModel queryModel, int index) {
      ...
      _queryParts.AddWherePart(
          "({0} = {1})",
          GetODataApiExpression(joinClause.OuterKeySelector),
          GetODataApiExpression(joinClause.InnerKeySelector));
    ...
    private string GetODataApiExpression(Expression expression) => ODataApiGeneratorExpressionTreeVisitor.GetODataApiExpression(expression);
