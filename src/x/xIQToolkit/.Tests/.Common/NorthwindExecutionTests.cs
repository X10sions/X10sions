...
namespace Test {
  ...
  public abstract class NorthwindExecutionTests : NorthwindTestBase {
    ...
    public void TestSingleFails() => Assert.Throws<Exception>(() => {
    ...
    public void TestSingleOrDefaultFails() => Assert.Throws<Exception>(() => {
    ...
    public void TestEqualLiteralNull() {
      ...
      Assert.Equal(true, GetProvider().GetQueryText(q.Expression).Contains("IS NULL"));
      var n = q.Count();
      Assert.Equal(1, n);
    }
    ...
    public void TestNotEqualLiteralNull() {
      var q = db.Customers.Select(c => c.CustomerID == "ALFKI" ? null : c.CustomerID).Where(x => x != null);
      Assert.Equal(true, GetProvider().GetQueryText(q.Expression).Contains("IS NOT NULL"));
      var n = q.Count();
      Assert.Equal(90, n);
    }
    ...
    public void TestConditionalResultsArePredicates() {
      var value = db.Orders.Where(c => c.CustomerID == "ALFKI").Max(c => (c.CustomerID == "ALFKI" ? string.Compare(c.CustomerID, "POTATO") < 0 : string.Compare(c.CustomerID, "POTATO") > 0));
    ...
    */
    ...
    public void TestCustomersIncludeOrders() {
      ...
      var nw = new Northwind(GetProvider().WithPolicy(policy));
    ...
    public void TestCustomersIncludeOrdersViaConstructorOnly() {
      ...
      var nw = new NorthwindX(GetProvider().WithPolicy(policy).WithMapping(mapping));
    ...
    public void TestOrdersIncludeDetailsWithFirst() {
      ...
      var ndb = new Northwind(GetProvider().WithPolicy(policy));
