...
namespace IQToolkit.Data.SQLite {
  ...
  public class SQLiteQueryProvider : DbEntityProvider {
    ...
    protected override DbEntityProvider New(DbConnection connection, QueryMapping mapping, QueryPolicy policy) => new SQLiteQueryProvider((SQLiteConnection)connection, mapping, policy);
    ...
    new class Executor : DbEntityProvider.Executor {
...
#else
        cmd = (SQLiteCommand)provider.Connection.CreateCommand();
        cmd.CommandText = query.CommandText;
        SetParameterValues(query, cmd, paramValues);
        cmd.Prepare();

        if (provider.Transaction != null) {
          cmd.Transaction = (SQLiteTransaction)provider.Transaction;
        }
#endif
        return cmd;
      }
      ...
      protected override void AddParameter(DbCommand command, QueryParameter parameter, object value) {
        ...
        var p = ((SQLiteCommand)command).Parameters.Add(parameter.Name, ((SqlQueryType)qt).SqlDbType.ToDbType(), qt.Length);
        if (qt.Length != 0) {
          p.Size = qt.Length;
        } else if (qt.Scale != 0) {
          p.Size = qt.Scale;
