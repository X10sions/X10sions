...
namespace RepoDb.StatementBuilders {
  public class DB2iSeries_MySqlStatementBuilder<TConnection> : BaseStatementBuilder where TConnection : DbConnection, IDbConnection {

    public DB2iSeries_MySqlStatementBuilder()
      : this(DbSettingMapper.Get(typeof(TConnection)), new DB2iSeriesConvertFieldResolver(), new ClientTypeToAverageableClientTypeResolver()) { }

    public DB2iSeries_MySqlStatementBuilder(IDbSetting dbSetting, IResolver<Field, IDbSetting, string> convertFieldResolver = null, IResolver<Type, Type> averageableClientTypeResolver = null)
    ...
    public override string CreateMerge(QueryBuilder queryBuilder,
      ...
      if (primaryField == null) {
        throw new PrimaryFieldNotFoundException($"DB2iSeries_MySql is using the primary key as qualifier for merge operation.");
      }
      ...
      if (qualifiers?.Any() == true) {
        ...
        if (others?.Any() == true) {
          throw new InvalidQualifiersException($"DB2iSeries_MySql is using the primary key as qualifier for merge operation. " +
