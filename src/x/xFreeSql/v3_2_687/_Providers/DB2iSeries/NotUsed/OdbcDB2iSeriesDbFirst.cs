...
class OdbcDB2iSeriesDbFirst : IDbFirst {
  IFreeSql _orm;
  protected CommonUtils _commonUtils;
  protected CommonExpression _commonExpression;
  public OdbcDB2iSeriesDbFirst(IFreeSql orm, CommonUtils commonUtils, CommonExpression commonExpression) {
    _orm = orm;
...
";
      ...
      foreach (var row in ds) {
        var object_id = int.Parse(string.Concat(row[0]));
...
";
      ...
      foreach (var row in ds) {
        ...
        var is_unique = bool.Parse(string.Concat(row[3]));
        var is_primary_key = bool.Parse(string.Concat(row[4]));
        var is_clustered = bool.Parse(string.Concat(row[5]));
...
";
        ...
        foreach (var row in ds) {
          ...
          var isThisSln = referenced_object_id != 0;
      ...
      foreach (var loc4 in loc2.Values) {
        ...
        loc4.Columns.Sort((c1, c2) => {
          var compare = c2.IsPrimary.CompareTo(c1.IsPrimary);
          if (compare == 0) {
            var b1 = loc4.ForeignsDict.Values.Where(fk => fk.Columns.Where(c3 => c3.Name == c1.Name).Any()).Any();
            var b2 = loc4.ForeignsDict.Values.Where(fk => fk.Columns.Where(c3 => c3.Name == c2.Name).Any()).Any();
