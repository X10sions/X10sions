...
class OdbcDB2iSeriesCodeFirst : Internal.CommonProvider.CodeFirstProvider {
  public override bool IsNoneCommandParameter { get => true; set => base.IsNoneCommandParameter = true; }
  public OdbcDB2iSeriesCodeFirst(IFreeSql orm, CommonUtils commonUtils, CommonExpression commonExpression) : base(orm, commonUtils, commonExpression) { }
  ...
  protected override string GetComparisonDDLStatements(params TypeAndName[] objects) {
    ...
    try {
      ...
      foreach (var obj in objects) {
        ...
        if (LocalExecuteScalar(tbname[0], $" select 1 from dbo.sysobjects where id = object_id(N'[{tbname[1]}].[{tbname[2]}]') and OBJECTPROPERTY(id, N'IsUserTable') = 1") == null) { //表不存在
          if (tboldname != null) {
...
use [" + database + "];", tboldname ?? tbname);
        ...
        if ((_commonUtils as OdbcDB2iSeriesUtils).ServerVersion > 9) //DB2iSeries 2008+
