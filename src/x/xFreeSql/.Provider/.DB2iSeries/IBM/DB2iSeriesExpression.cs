...
class DB2iSeriesExpression : CommonExpression {

  public DB2iSeriesExpression(CommonUtils common) : base(common) { }
        ...
        case "Join":
          if (exp.IsStringJoin(out var tolistObjectExp, out var toListMethod, out var toListArgs1)) {
            var newToListArgs0 = Expression.Call(tolistObjectExp, toListMethod,
                Expression.Lambda(
                    Expression.Call(
                        typeof(SqlExtExtensions).GetMethod("StringJoinDB2iSeriesGroupConcat"),
