...
class DB2iSeriesCodeFirst : Internal.CommonProvider.CodeFirstProvider {

    public DB2iSeriesCodeFirst(IFreeSql orm, CommonUtils commonUtils, CommonExpression commonExpression) : base(orm, commonUtils, commonExpression) { }
  ...
  protected override string GetComparisonDDLStatements(params TypeAndName[] objects) {
    ...
    foreach (var obj in objects) {
      ...
      var isIndent = false;
      if (_orm.Ado.ExecuteScalar(CommandType.Text, $" select 1 from {tbname[0]}.DB2iSeries_master where type='table' and name='{tbname[1]}'") == null) { //表不存在
        if (tboldname != null) {
          if (_orm.Ado.ExecuteScalar(CommandType.Text, $" select 1 from {tboldname[0]}.DB2iSeries_master where type='table' and name='{tboldname[1]}'") == null)
            //模式或表不存在
            tboldname = null;
        }
        if (tboldname == null) {
      ...
      var dsql = _orm.Ado.ExecuteScalar(CommandType.Text, $" select sql from {tbtmp[0]}.DB2iSeries_master where type='table' and name='{tbtmp[1]}'")?.ToString();
      ...
      if (istmpatler == false) {
        ...
        foreach (var dbIndex in dbIndexes) {
          ...
          var dbIndexesSql = string.Concat(_orm.Ado.ExecuteScalar(CommandType.Text, $" SELECT sql FROM {_commonUtils.QuoteSqlName(tbtmp[0])}.DB2iSeries_master WHERE name = '{dbIndex[1]}'"));
