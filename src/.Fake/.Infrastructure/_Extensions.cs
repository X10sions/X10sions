...
namespace X10sions.Fake;
 static class MethodInfoFor {
  static MethodInfo? GetMethodInfo(this Type type, string name) => type.GetMethods().FirstOrDefault(x => x.Name == name);
  internal static MethodInfo? GetWholeYearsBetween{ get; } = typeof(DomainExtensions).GetMethodInfo(nameof(DomainExtensions.GetWholeYearsBetween));
}

public static class InfacstructureExtensions {
  ...
  public static LambdaExpression GetWholeYearsBetween_LambdaExpr() => DelegateDecompiler.MethodBodyDecompiler.Decompile(MethodInfoFor.GetWholeYearsBetween);
  public static Expression<Func<DateTime?, DateTime, int?>> GetWholeYearsBetween_Expr() => (Expression<Func<DateTime?, DateTime, int?>>)DelegateDecompiler.MethodBodyDecompiler.Decompile(MethodInfoFor.GetWholeYearsBetween);
