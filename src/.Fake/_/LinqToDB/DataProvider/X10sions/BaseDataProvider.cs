...
namespace LinqToDB.DataProvider.X10sions {
  public abstract class BaseDataProvider<TConn, TDataReader> : IDataProvider where TConn : DbConnection, new() where TDataReader : DbDataReader {
    public BaseDataProvider(string name, int id, IDataProvider baseDataProvider) {
      Name = name;
      ID = id;
      this.baseDataProvider = baseDataProvider;
      BaseMappingSchema = baseDataProvider.MappingSchema;
      BaseMappingSchema.SetDataType(typeof(string), new SqlDataType(DataType.VarChar, typeof(string), 255));
    }

    MappingSchema BaseMappingSchema;
    ...
    public MappingSchema MappingSchema => BaseMappingSchema;
