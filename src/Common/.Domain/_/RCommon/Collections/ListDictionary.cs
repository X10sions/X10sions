...
public class ListDictionary<TKey, TValue> : IEnumerable<KeyValuePair<TKey, List<TValue>>> {
  ...
  public void Clear() => innerValues.Clear();
  ...
  public IEnumerable<TValue> FindByKeyAndValue(Predicate<TKey> keyFilter, Predicate<TValue> valueFilter) {
    foreach (KeyValuePair<TKey, List<TValue>> pair in this)
      if (keyFilter(pair.Key))
        foreach (TValue value in pair.Value)
  ...
  public IEnumerator<KeyValuePair<TKey, List<TValue>>> GetEnumerator() => innerValues.GetEnumerator();

  IEnumerator IEnumerable.GetEnumerator() => innerValues.GetEnumerator();
