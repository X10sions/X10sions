...
public abstract class BusinessEntity : IBusinessEntity {
  ...
  /// <inheritdoc/>
  public override string ToString() => $"[ENTITY: {GetType().Name}] Keys = {GetKeys().GetDelimitedString(',')}";

  public abstract object[] GetKeys();

  public bool EntityEquals(IBusinessEntity other) => this.BinaryEquals(other);
  ...
  public void AddLocalEvent(ISerializableEvent eventItem) {
    ...
    OnLocalEventsAdded(new TransactionalEventsChangedEventArgs(this, eventItem));
  }

  public void RemoveLocalEvent(ISerializableEvent eventItem) {
    _localEvents?.Remove(eventItem);
    OnLocalEventsRemoved(new TransactionalEventsChangedEventArgs(this, eventItem));
  }

  public void ClearLocalEvents() {
    _localEvents?.Clear();
    OnLocalEventsCleared(new TransactionalEventsClearedEventArgs(this));
  }
...
[Serializable]
public abstract class BusinessEntity<TKey> : BusinessEntity, IBusinessEntity<TKey> where TKey : IEquatable<TKey> {
  /// <inheritdoc/>
