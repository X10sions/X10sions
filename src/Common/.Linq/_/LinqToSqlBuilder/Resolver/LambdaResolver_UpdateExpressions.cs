...
partial class LambdaResolver {
    ...
    if (assignmentExpression.Expression is BinaryExpression expression) {
      switch (assignmentExpression.Expression.NodeType) {
        case ExpressionType.Add:
          Builder.UpdateFieldWithOperation(GetColumnName(expression.Left), expression.Right.GetExpressionValue(), "+");
          break;
  ...
  private void ResolveUpdateMethodCall(MethodCallExpression callExpression) {
    var arguments = callExpression.Arguments.Select(LinqExtensions.GetExpressionValue).ToArray();
