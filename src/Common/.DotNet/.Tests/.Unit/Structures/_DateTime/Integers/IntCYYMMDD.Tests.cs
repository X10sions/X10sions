...
public class IntCYYMMDDTests(ITestOutputHelper testOutputHelper) {
  ...
  public void ConstructorTests_GivenEmpty_ShouldBeToday() {
    ...
    var expected = TimeProvider.System.GetLocalNow().DateTime.Date;
  ...
  public record GivenDateOnly(DateOnly DateOnly, int ExpectedValue, int ExpectedC, int ExpectedYY, int ExpectedMM, int ExpectedDD) {
    ...
    public static TheoryData<GivenDateOnly> TheoryData { get; } = new TheoryData<GivenDateOnly> {
      new (new(  10,  1,  1),   10101,  0,  1,  1,  1),
      new (new( 100,  1,  1),   10101,  0,  1,  1,  1),
      new (new(1000,  1,  1),   10101,  0,  1,  1,  1),
      new (new(1900,  1,  1),   10101,  0,  1,  1,  1),
      new (new(1901,  1,  1),   10101,  0,  1,  1,  1),
      new (new(1999,  7, 24),  990724,  0, 99,  7, 24),
      new (new(1999,  7, 31),  990799,  0, 99,  7, 99),
      new (new(2001,  1,  1), 1010101,  1,  1,  1,  1),
      new (new(2017,  8, 31), 1170831,  1, 17,  8, 31),
      new (new(2020,  2, 29), 1200229,  1, 20,  2, 29),
      new (new(2021,  2, 28), 1210229,  1, 21,  2, 28),
      new (new(2021,  2, 28), 1210229,  1, 21,  2, 29),
      new (new(2024,  1,  1), 1240101,  1, 24,  1,  1),
      new (new(2024, 11,  1), 1241100,  1, 24, 11,  0),
      new (new(2024, 11, 30), 1241130,  1, 24, 11, 30),
      new (new(2024, 11, 30), 1241131,  1, 24, 11, 31),
      new (new(2024, 12,  1), 1241201,  1, 24, 12,  1),
      new (new(2024, 12, 31), 1241231,  1, 24, 12, 31),
      new (new(9999, 12, 31), 9999999,  9, 99, 99, 99),
    };
  }

  [Theory, MemberData(nameof(GivenDateOnly.TheoryData), MemberType = typeof(GivenDateOnly))]
  public void ConstructorTests_GivenDateOnly(GivenDateOnly data) {
  ...
  [Theory, MemberData(nameof(GivenDateOnly.TheoryData), MemberType = typeof(GivenDateOnly))]
  public void ConstructorTests_GivenDateTime(GivenDateOnly data) {
    var sut = new IntCYYMMDD(data.DateOnly.ToDateTime(TimeOnly.MinValue));
    using (new AssertionScope()) {
      sut.DateOnly.Should().Be(data.DateOnly);
      sut.DateTime.Should().Be(data.DateOnly.ToDateTime(TimeOnly.MinValue));
      sut.C.Should().Be(data.ExpectedC);
