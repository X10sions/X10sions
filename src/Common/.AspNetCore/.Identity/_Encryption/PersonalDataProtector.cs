...
namespace Common.AspNetCore.Identity {
  ...
  public class PersonalDataProtector : IPersonalDataProtector {
    // https://github.com/blowdart/AspNetCoreIdentityEncryption/blob/master/AspNetCoreIdentityEncryption/PersonalDataProtector.cs
    ...
    public string Unprotect(string data) {
      ...
      var computedSignature = SignData(cipherTextAndIV, masterKey, encryptingAlgorithm, signingAlgorithm, keyDerivationIterationCount);
    ...
    public byte[] Key(string keyId) => Convert.FromBase64String(_keyRing[keyId]);
    ...
    private byte[] DerivedSigningKey(byte[] key, SymmetricAlgorithm algorithm, int keyDerivationIterationCount) => KeyDerivation.Pbkdf2(
          @"PersonalDataSigning",
          key,
    ...
    private byte[] DerivedEncryptionKey(byte[] key, SymmetricAlgorithm algorithm, int keyDerivationIterationCount) => KeyDerivation.Pbkdf2(
