...
public static class _Extensions {
  ...
  public static bool IsNullable(this PropertyInfo property) => property.GetMethod is not null && new NullabilityInfoContext().Create(property.GetMethod.ReturnParameter).ReadState == NullabilityState.Nullable;
...
[PreDotNet6Compatibility, Obsolete("introduced in .NET 6")]
...
[PreDotNet6Compatibility, Obsolete("introduced in .NET 6")]
public enum NullabilityState { Unknown = 0, NotNull = 1, Nullable = 2 }
...
public sealed class NullabilityInfoContext {
  ...
  internal static bool IsSupported { get; } = !AppContext.TryGetSwitch("System.Reflection.NullabilityInfoContext.IsSupported", out var isSupported) || isSupported;
  ...
  private bool IsPrivateOrInternalMethodAndAnnotationDisabled(MethodBase method) => (method.IsPrivate || method.IsFamilyAndAssembly || method.IsAssembly) &&
  ...
  private bool IsPrivateOrInternalFieldAndAnnotationDisabled(FieldInfo fieldInfo) => (fieldInfo.IsPrivate || fieldInfo.IsFamilyAndAssembly || fieldInfo.IsAssembly) &&
  ...
  private bool IsPublicOnly(bool isPrivate, bool isFamilyAndAssembly, bool isAssembly, Module module) {
    ...
    return value != NotAnnotatedStatus.None
  ...
  private bool TryPopulateNullabilityInfo(NullabilityInfo nullability, NullableAttributeStateParser parser, ref int index) {
    var isValueType = IsValueTypeOrValueTypeByRef(nullability.Type);
    if (!isValueType) {
      var state = NullabilityState.Unknown;
      if (!parser.ParseNullableState(index, ref state)) {
        return false;
