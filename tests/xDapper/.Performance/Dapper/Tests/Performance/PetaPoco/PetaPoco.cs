...
namespace PetaPoco {
  ...
  public class Database : IDisposable {
    ...
    private bool IsMySql() => string.Compare(_providerName, "MySql.Data.MySqlClient", true) == 0;
    private bool IsSqlServer() => string.Compare(_providerName, "System.Data.SqlClient", true) == 0;
    ...
    public static string ProcessParams(string _sql, object[] args_src, List<object> args_dest) => rxParams.Replace(_sql, m => {
    ...
    public Page<T> Page<T>(long page, long itemsPerPage, string sql, params object[] args) where T : new() {
      ...
      if (EnableAutoSelect)
        sql = AddSelectClause<T>(sql);
      if (!SplitSqlForPaging(sql, out var sqlCount, out var sqlSelectRemoved, out var sqlOrderBy))
    ...
    public Page<T> Page<T>(long page, long itemsPerPage, Sql sql) where T : new() => Page<T>(page, itemsPerPage, sql.SQL, sql.Arguments);
    ...
    public List<T> Fetch<T>(Sql sql) where T : new() => Fetch<T>(sql.SQL, sql.Arguments);

    public IEnumerable<T> Query<T>(Sql sql) where T : new() => Query<T>(sql.SQL, sql.Arguments);

    public T Single<T>(string sql, params object[] args) where T : new() {
      var val = SingleOrDefault<T>(sql, args);
    ...
    public T First<T>(string sql, params object[] args) where T : new() {
      var val = FirstOrDefault<T>(sql, args);
      if (!EqualityComparer<T>.Default.Equals(val, default(T)))
        return val;
      else
        throw new InvalidOperationException("The sequence contains no elements");
    ...
    public T Single<T>(Sql sql) where T : new() => Single<T>(sql.SQL, sql.Arguments);

    public T SingleOrDefault<T>(Sql sql) where T : new() => SingleOrDefault<T>(sql.SQL, sql.Arguments);

    public T FirstOrDefault<T>(Sql sql) where T : new() => FirstOrDefault<T>(sql.SQL, sql.Arguments);

    public T First<T>(Sql sql) where T : new() => First<T>(sql.SQL, sql.Arguments);
    ...
    public int Update(string tableName, string primaryKeyName, object poco) => Update(tableName, primaryKeyName, poco, null);

    public int Update(object poco) => Update(poco, null);
    ...
    public int Delete(string tableName, string primaryKeyName, object poco) => Delete(tableName, primaryKeyName, poco, null);
    ...
    public string LastCommand {
      get {
        ...
        if (_lastArgs != null) {
          sb.Append("\r\n\r\n");
          for (var i = 0; i < _lastArgs.Length; i++) {
    ...
    internal class PocoData {
      public static PocoData ForType(Type t) {
        lock (m_PocoData) {
          if (!m_PocoData.TryGetValue(t, out var pd)) {
            pd = new PocoData(t);
            m_PocoData.Add(t, pd);
          }
          return pd;
      ...
      public PocoData(Type t) {
        ...
        var ExplicitColumns = t.GetCustomAttributes(typeof(ExplicitColumns), true).Length > 0;
      ...
      public Func<IDataReader, T> GetFactory<T>(string key, bool ForceDateTimesToUtc, IDataReader r) {
        lock (PocoFactories) {
          ...
          if (PocoFactories.TryGetValue(key, out var factory))
          ...
          lock (m_Converters) {
            ...
            for (var i = 0; i < r.FieldCount; i++) {
              ...
              if (!Columns.TryGetValue(r.GetName(i), out var pc))
                continue;
    ...
    private class ShareableConnection : IDisposable {
      ...
      public void Dispose() => _db.CloseSharedConnection();
    }
  ...
  public class Sql {
    ...
    public Sql Append(string sql, params object[] args) => Append(new Sql(sql, args));

    public Sql Where(string sql, params object[] args) => Append(new Sql("WHERE " + sql, args));

    public Sql OrderBy(params object[] args) => Append(new Sql("ORDER BY " + string.Join(", ", (from x in args select x.ToString()).ToArray())));

    public Sql Select(params object[] args) => Append(new Sql("SELECT " + string.Join(", ", (from x in args select x.ToString()).ToArray())));

    public Sql From(params object[] args) => Append(new Sql("FROM " + string.Join(", ", (from x in args select x.ToString()).ToArray())));

    private static bool Is(Sql sql, string sqltype) => sql?._sql != null && sql._sql.StartsWith(sqltype, StringComparison.InvariantCultureIgnoreCase);
